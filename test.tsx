...
//! ЛИТЕРАЛЬНЫЕ ТИПЫ - значение к примеру type задаётся строго

let s: 'test' = 'test' // теперь s строго принимает только test

//Пример

type action = 'up' | 'down';

function perform(params: action): -1|1 {
    switch (params) {
        case 'up':
            return 1     
        case 'down':
...
//! ENUMS - с помощью их мы можем ограничить область значений переменной 

enum Direction1 {
    Up, // задаётся значение индекса автоматически
    Down,
    Left,
    Right
}

// или можно задать string
...
// РАСЧЁТНЫЙ ENUM, может быть только числовым, но не строковым!
...
//! Generics это инструмент написания кода, который можно переиспользовать
// Они позволяют использовать ф-ции и объеты для разных типов данных

function Example22<T>(params:T):T {
    if (typeof params == 'string'){
        params.toLocaleUpperCase()
...
Example22<string>('stroka')
Example22<number>(22) 

interface Iinterface {
   Example: <T,F>(a:number) => F // <T, F> перечисл. всех значений которые исп. в ф-ции
}

// Также можно передать в generic и классы
...
const c1 = new Hog<string>('sdf')


//? Уточнение параметра типа 

// если мы хотим передать тип в ф-цию с определёнными параметрами то можно использовать
// интерфейс, где можно указать значения, которые должны быть обязательными

interface Length {
    length:number
}

function sdfs<T extends Length>(param:T):number {
    return param.length
}

sdfs([2]) // у массива есть св-во length, поэтому его передать можно. 
// Но не обязательно чтобы это было именно св-во, может быть объект c данным св-вом
// Например
sdfs({asd:'sdf',length:1})



function getProperty<T,K extends keyof T>(obj:T,key:K) { // K extends keyof T обязательная проверка ключа на содержании в объекте
    return obj[key] // Возвр Т[K], где K ключ объекта T
}
const myObj = {
    a:1,
    b:2,
...
//! Настройка JSX
// 1) Чтобы писать на jsx, нужно расширение файла tsx или jsx
// 2) В config.json выставить след. папрметры:
// "jsx": "react"
